...
# Discover interfaces
cu_if=$(get_veth_ifname_for_container rfsim5g-oai-cu1) # adapt the cu container name if needed
du_if=$(get_veth_ifname_for_container rfsim5g-oai-du1) # adapt the du container name if needed
...
MOBIFLOW_COMMAND="$cu_if $du_if" docker-compose up -d mobiflow-agent-1