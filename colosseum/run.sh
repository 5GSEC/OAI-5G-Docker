...
while [ -n "$1" ]; do
    ...
    case "$_arg" in
        ...
        1|lteue)
            ...
            _common_args="-O $_config_path -C 2680000000 -r 25 --ue-scan-carrier --nokrnmod 1 --noS1 --ue-rxgain 120 --ue-txgain 30 --ue-max-power 0 --ue-nb-ant-tx 1 --ue-nb-ant-rx 1 --usrp-args \"$_usrp_args\" -d 2>&1 --opt.type pcap --opt.path $_pcap_path/LTEUE-$(date +"%m%d%H%M").pcap | tee ../../../../mylogs/LTEUE-$(date +"%m%d%H%M").log"
        ...
        2|gnb)
            _find_route=true
            _config_path="/root/OAI-5G-Docker/nr-usrp/gnb.conf"
            _common_args="-O $_config_path --sa -E --gNBs.[0].min_rxtxtime 6 --usrp-tx-thread-config 1 -E --continuous-tx 1 2>&1 --opt.type pcap --opt.path $_pcap_path/GNB-$(date +"%m%d%H%M").pcap | tee ../../../../mylogs/GNB-$(date +"%m%d%H%M").log"
        ...
        3|gnb-cu)
            ...
            _common_args="-O $_config_path --sa -E --gNBs.[0].min_rxtxtime 6 2>&1 --opt.type pcap --opt.path $_pcap_path/GNB-CU-$(date +"%m%d%H%M").pcap | tee ../../../../mylogs/GNB-CU-$(date +"%m%d%H%M").log"
            _exec_path="./nr-softmodem"
            #_cmd="numactl --cpunodebind=netdev:usrp0 --membind=netdev:usrp0 ./nr-softmodem $_common_args"
            ;;
        4|gnb-du)
            ...
            _common_args="-O $_config_path --sa -E --gNBs.[0].min_rxtxtime 6 --usrp-tx-thread-config 1 -E --continuous-tx 1 2>&1 --opt.type pcap --opt.path $_pcap_path/GNB-DU-$(date +"%m%d%H%M").pcap | tee ../../../../mylogs/GNB-DU-$(date +"%m%d%H%M").log"
            _exec_path="./nr-softmodem"
            #_cmd="numactl --cpunodebind=netdev:usrp0 --membind=netdev:usrp0 ./nr-softmodem $_common_args"
            ;;
        5|nrue*)
